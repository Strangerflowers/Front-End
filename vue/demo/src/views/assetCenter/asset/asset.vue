<template>
  <div class="animated fadeIn col-lg-10-paddingleft0">
    <b-row>
      <b-col lg="12">
        <b-card class="top-search-wrapper">
          <b-form inline :model="form">
            ...
            <b-form-group  label="资产状态："  label-for="assetStatus" >
                <b-form-select id="assetStatus" v-model="form.propertyStatus"   :plain="true"  :options="assetStatusArr">

                </b-form-select>
            </b-form-group>

            <b-form-group  label="租用状态："  label-for="assetPostal" >
                <b-form-select id="assetPostal" v-model="form.assetPostal"   :plain="true"  :options="assetPostalArr">

                </b-form-select>
            </b-form-group>

            <b-form-group  label="单元："  label-for="assetSimpleName"  >
              <b-form-input id="assetSimpleName" type="text" v-model="form.assetSimpleName" @keyup.enter="submitBtn"></b-form-input>
            </b-form-group>

          </b-form>
          <div class="btns-box">
            <b-button @click="submitBtn" type="submit" size="sm" variant="primary"><i class="fa fa-dot-circle-o"></i> 查询</b-button>
            <b-button @click="resetBtn" type="reset" size="sm" variant="danger"><i class="fa fa-ban"></i> 重置</b-button>
          </div>
           <div class="add">
            <imgUpload :src="'/api/imgs'" ></imgUpload>
          </div>
    ...
    <b-row>
      <b-col lg="12" class="top-btns">
        <b-button @click="addFun"  size="sm" variant="primary"><i class="fa fa-plus-square-o"></i> 新增</b-button>
      </b-col>
    </b-row>
    <b-row>
      <b-col lg="2">
        <z-el-tree @currentNode='getTreeNode' :reUploadId="reloadTreeDataById"></z-el-tree>
      </b-col>
      <b-col lg="10">

        <!--table表格组件-->
        <c-table  hover striped bordered :topBtn="true" :busy.sync="isBusy" :table-data="tableData" :fields="fields" :totalRows='totalRows' :perPage="perPage"  @getSortType="getSortType"  @updatePage="updatePage" @getSelectedRows="getSelectedRows">

          <!--table表格操作按钮-->
          <template v-slot:control = "row">
            <i :id="'popoverButton-'+row.item.id" class="fa fa-ellipsis-h" ></i>
            <b-popover  :target="'popoverButton-'+row.item.id" placement='buttom' triggers="hover focus">
              <template>
                <b-button variant="primary" size="sm" @click="updateAssetFun(row.item.id)" :disabled ="row.item.status== '1'?true:false">编辑</b-button>
                <!-- <b-button variant="primary" size="sm" @click="detailAssetFun(row.item.id)">查看</b-button> -->
                <b-button variant="primary" v-if="row.item.status== '0' " size="sm" @click="enableAssetFun(row.item.id,row.item.status,row.item.assetType)">启用</b-button>
                <b-button variant="primary" v-if="row.item.status== '1' " size="sm" @click="disableAssetFun(row.item.id,row.item.status,row.item.assetType)">停用</b-button>
              </template>
            </b-popover>
          ...
          <template v-slot:perPageSetting >
            <b-form inline class="perPage-setting">
              <b-form-group  label="每页"  label-align-sm="right"  label-for="perPageSelect">
                <b-form-select  v-model="perPage"  id="perPageSelect"  size="sm"  :options="pageOptions"  ></b-form-select>行
              </b-form-group>
            </b-form>
          </template>
          <!--/table表格每页显示行数设置-->
    ...
    <b-modal :id="infoModal.id" :title="infoModal.title" size="xl"  @hide="resetInfoModal" hide-footer hide-header-close no-close-on-backdrop >
      <div slot="modal-title"><i class="fa fa-align-justify"></i>查看</div>
      ...
      watch:{
        ...
        methods:{
          ...
          getTreeNode(nodeData){
            ...
            this.assetTreeInfo.treeType=nodeData.data.assetType
            this.assetTreeInfo.treeName=nodeData.data.assetSimpleName
            this.assetTreeInfo.treeId=nodeData.data.id
            this.assetTreeInfo.treeStatus=nodeData.data.status
            if(this.form.assetSimpleName!=''){
              this.getTableData("unit");
            }else {
          ...
          addFun(){
            ...
            let that=this;
            if(this.assetTreeInfo.treeId=='' && this.assetTreeInfo.treeType==''){
              that.makeToast("请先选择树节点")
              return;
            }
            if(this.assetTreeInfo.treeStatus==null || this.assetTreeInfo.treeStatus==0){
              that.makeToast("此资产已停用，不能新增下级")
              return;
            }
            this.addOrUpdateModal.targetObj=this.assetTreeInfo   //tree节点id信息
          ...
          enableAssetFun(assetId,status,assetType){
            ...
            .then( res=> {
              if (res.data.code === 0) {
                that.makeToast("启用成功")
                if(this.assetTreeInfo.treeType==undefined || this.assetTreeInfo.treeType==''){
                  this.getTableData(assetTypeParent.get(assetType));
                }else {
                  this.getTableData(this.assetTreeInfo.treeType);
                }
                this.reloadTreeDataById=this.assetTreeInfo.treeId
              } else {
          ...
          getTableColumnShow(treeType){
            ...
            switch (treeType) {
              case "hq":
                columnShow=[
                  ...
                  {key: 'ifCalculation',label: '配套是否计租', formatter:(value)=>{
              ...
              case "unit":
                columnShow=[
                  ...
                  {key: 'industryRegister',label: '是否已工商备案', formatter:(value)=>{
                      if(value!=undefined && value!=null ){
                        return  value==0 ? '否' :'是';
                      }
                    }},
                  ...
                  {key: 'isSupPower',label: '是否有供电(招牌)', formatter:(value)=>{
                      if(value!=undefined && value!=null ){
                        return  value==0 ? '否' :'是';
                      }
                    }},
            ...
            let fixed=[
              {key: 'status',label: '状态', formatter:(value)=>{
                  if(value!=undefined && value!=null){
...
<style lang="scss">
  ...
  .input-group-append .fa,.input-group-append .btn{
    margin-right: 0;
  }
  ...
  .inline-form-col-6 .custom-select, .inline-form-col-6 .form-control, .popover-container .form-group{
    width: 100%;
  }
  .popover-container .form-control,.input-group .form-control{
    width: 1%;
  }

  .input-group > .input-group-append:not(:last-child) > .input-group-text,.input-group > .input-group-append:not(:last-child) > .btn{
